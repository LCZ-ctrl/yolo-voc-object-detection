...
def postprocess_yolov2(bboxes, scores, conf_thresh=0.01, nms_thresh=0.5):
    """
...
@torch.no_grad()
def predict_yolov2_batch(model, images, conf_thresh=0.01, nms_thresh=0.5):
    """
    ...
    obj_pred = model.obj_pred(reg_feat)
    ...
    # [B, A*C, H, W] -> [B, H*W*A, C]
    cls_pred = cls_pred.permute(0, 2, 3, 1).contiguous().view(bs, -1, model.num_classes)
    # [B, A*4, H, W] -> [B, H*W*A, 4]
    reg_pred = reg_pred.permute(0, 2, 3, 1).contiguous().view(bs, -1, 4)

    # anchors: [M, 4], M = H * W * A
    anchors = model.generate_anchors(fmp_size).to(device)

    # decode boxes using the same logic as the model
    pred_box = model.decode_boxes(anchors, reg_pred)  # [B, M, 4]
    ...
    for i in range(bs):
        ...
        boxes, box_scores, labels = postprocess_yolov2(
