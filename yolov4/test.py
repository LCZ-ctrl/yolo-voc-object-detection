...
if __name__ == '__main__':
    ...
    model = YOLOv4(device=device, num_classes=config.num_classes, anchor_size=config.anchor_size, trainable=False,
                   topk=config.topk, model_name='cspdarknet53', pretrained=config.pretrained).to(device)
    weight_path = Path(config.save_folder) / 'yolov4_voc_best.pth'
